...
univar <- function(dataset, var, plots = TRUE,
                    font_family = "Lato",
  ...
  if (!is.numeric(dataset %>% pull({{ var }}))) {
    ...
    out[["summary"]] <- tibble(
      stat  = c("min", "mean", "median", "max", "sd", "var",
                "n_obs", "n_missing"),
      value = c(
        dataset %>% pull({{ var }}) %>% min(., na.rm = T),
        dataset %>% pull({{ var }}) %>% mean(., na.rm = T),
        dataset %>% pull({{ var }}) %>% median(., na.rm = T),
        dataset %>% pull({{ var }}) %>% max(., na.rm = T),
        dataset %>% pull({{ var }}) %>% sd(., na.rm = T),
        dataset %>% pull({{ var }}) %>% stats::var(., na.rm = T),
        dataset %>% pull({{ var }}) %>% na.omit() %>% length(),
        dataset %>% pull({{ var }}) %>% is.na() %>% sum()
      )
    )
    
    out[["quantiles"]] <- quantile(dataset %>% pull({{ var }}) %>% na.omit(),
                                   probs = percentiles) %>%
